...
kmer_data = pd.DataFrame({"k-mer": kmers, \
 'Coverage':[0] * len(kmers), \
 'Methylated_10':[0] * len(kmers), \
 'Methylated_20':[0] * len(kmers),\
 'Methylated_30':[0] * len(kmers),\
 'Methylated_40':[0] * len(kmers),\
 'Methylated_50':[0] * len(kmers),\
 'Methylated_60':[0] * len(kmers),\
 'Methylated_70':[0] * len(kmers),\
 'Methylated_80':[0] * len(kmers),\
 'Methylated_90':[0] * len(kmers)})
...
for read in bamfile.fetch():
    ...
    for index in range(read.reference_start, read.reference_end - 5):

        if index not in list(list(zip(*aligned_positions))[1]):
            continue

        # Index is first position of kmer
        read_kmer = genome_data[read.reference_name][index:index + 6]

        if read_kmer not in kmers:
            continue
            
        # Select a random A from the kmer to look at
        a_pos = []
        for idx, value in enumerate(read_kmer):
            if genome_data[read.reference_name][index + idx] == "A":
                a_pos.append(index + idx)

        selected_a = random.sample(a_pos, 1)

        [(query_index, ref_index)] = [item for item in aligned_positions if item[1] == selected_a[0]] 

        # If position of interest exists in aligned query sequence
        if query_index != None:
                    
            # Get index of kmer in kmer_data pandas dataframe
            idx = kmer_data.index[kmer_data["k-mer"] == read_kmer]
                    
            # Check if have enough sites for this kmer yet
            if kmer_data.iat[idx[0], 1] == 1000:
                print("Completed :", read_kmer)
                continue
...
# Write to a bed file
if not args.nonbarcoded:
    kmer_data.to_csv(args.outfile + args.barcode + ".bed", sep = '\t', index = False)

else:
    kmer_data.to_csv(args.outfile + ".bed", sep = '\t', index = False)
