...
for (barcode in c("15", "17", "18")){
  print(barcode)
  data <- read.csv(paste0("barcode", barcode, "/barcode", barcode, "_EcoGII_Jiang_nucleosomes_150bp_distance_combined.bedtools_closest", sep=""), header = FALSE, sep = "\t")
  colnames(data) <- c("Chr", "Start", "End", "ReadID", "Methylation", "Strand", "Nuc_chr", "Nuc_start", "Nuc_end", "Distance")
...
p <- ggplot(counts, aes(x = Distance, y = Total, colour = Barcode)) + 
  ...
  geom_line() + 
  labs(title = "Number of adenines around Jiang nucleosomes", x = "Bases from ACS", y =  "Number of adenines") +
...
print(p)
ggsave("EcoGII_Jiang_nucleosomes_methylation_counts.png", width = 8)
...
p <- ggplot(meta_wide, aes(x = Distance, y = Percentage_methylated, colour = Barcode)) + 
  ...
  geom_line() + 
  labs(title = "Adenine methylation around Jiang nucleosomes", x = "Bases from ACS", y =  "Percentage of sites above 70% threshold (%)") +
  scale_colour_manual(values = c("#000000", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")) + 
  facet_grid(vars(Barcode)) +
  theme(panel.spacing = unit(2, "lines")) +
  theme_bw() + 
...
print(p)
ggsave("EcoGII_Jiang_nucleosomes_methylation_above_70_combined.png", width = 8)
