...
p <- ggplot(counts, aes(x = Distance, y = Total, colour = Barcode)) + 
  ...
  geom_line() + 
  labs(title = "Number of adenines around Eaton origins", x = "Bases from ACS", y =  "Number of adenines") +
...
print(p)
ggsave("EcoGII_Berbenetz_origins_methylation_counts.png", width = 8)
...
p <- ggplot(meta_wide, aes(x = Distance, y = Percentage_methylated, colour = Barcode)) + 
  ...
  geom_line() + 
  labs(title = "Adenine methylation around Eaton origins", x = "Bases from ACS", y =  "Percentage of sites above 70% threshold (%)") +
  scale_colour_manual(values = c("#000000", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")) + 
  facet_grid(vars(Barcode)) +
  theme(panel.spacing = unit(2, "lines")) +
  theme_bw() + 
...
print(p)
ggsave("EcoGII_Berbenetz_origins_methylation_above_70_combined.png", width = 8)
