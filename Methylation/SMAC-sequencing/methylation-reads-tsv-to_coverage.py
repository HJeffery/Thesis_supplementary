...
def run():
    ...
    while line != '':
        ...
        if doBI:
            ...
            for i in range(len(Ps)):
                ...
                p = float(LLs[i])
                PDict[pos] = p
            positions = list(PDict.keys())
            minPos = int(min(positions))
            maxPos = int(max(positions))
            if doSaveNewFile:
                NewPos = []
                NewLLs = []
            for pos in range(minPos + int(window/2), maxPos - int(window/2), int(step)):
                (A,B) = (alph,bet)
                for j in range(pos - int(window/2), pos + int(window/2)):
                    if j in PDict:
                        p = PDict[j]
                        if doAweight:
    ...
    chromosomes = list(CoverageDict.keys())
    chromosomes.sort()
    ...
    for chr in chromosomes:
        print(chr)
        positions = list(CoverageDict[chr].keys())
        positions.sort()
